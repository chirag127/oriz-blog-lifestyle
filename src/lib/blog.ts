...
/**
 * Content themes — collapse raw category strings into the four "day-arc"
 * themes this lifestyle notebook covers. Drives the marker colour (--v-*) on
 * rows and category kickers so Mornings / Space / Focus / Slow living read
 * differently at a glance. The internal keys keep the base component names
 * (dev/finance/culture/health/personal) so no chrome needs touching — only
 * the labels + colours (via tokens.css aliases) change.
 *   dev → Focus · finance → Space · culture → Slow living · health → Mornings
 */
...
/** Map a raw category string → one of the day-arc themes. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(morning|routine|rise|wake|sleep|rhythm|ritual)/.test(c)) return 'health'
  if (/(space|declutter|minimal|less|own|stuff|home|things|clutter)/.test(c)) return 'finance'
  if (/(focus|work|attention|deep|distract|mind|productiv)/.test(c)) return 'dev'
  if (/(slow|living|life|simple|calm|lifestyle|habit|self)/.test(c)) return 'culture'
  return 'personal'
}
